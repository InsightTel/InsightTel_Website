...
// ============== HELPER: BACA REKOMENDASI DARI LOCALSTORAGE ==============
function getRecommendedIdsFromStorage() {
    try {
        const raw = localStorage.getItem('insighttel_recommendedProducts');
        if (!raw) return [];
        const parsed = JSON.parse(raw);
        if (!Array.isArray(parsed)) return [];
        return parsed.map(id => String(id));
    } catch (e) {
        console.error('Gagal baca insighttel_recommendedProducts dari localStorage:', e);
        return [];
    }
}
...
const productRenderer = {
    render: () => {
        ...
        // Baca ID produk yang direkomendasikan dari localStorage
        const recommendedIds = getRecommendedIdsFromStorage();
        ...
        const filteredProducts = products.filter(product => {
            ...
            const matchesSearch = !q || 
                ...
                product.features.some(feat => feat.toLowerCase().includes(q)) ||
        ...
        filteredProducts.forEach(product => {
            ...
            // Cek apakah produk ini direkomendasikan (berdasarkan localStorage)
            const isRecommended = recommendedIds.includes(String(product.id));
            ...
            // Tambahkan highlight kalau direkomendasikan
            if (isRecommended) {
