...
async function getRecommendationsFromAPI() {
    ...
    // --- ENCODING ---
    // Konversi nilai string dari form menjadi angka yang diharapkan oleh model ML
    const encodingMap = {
        planType: { basic: 0, premium: 1 },
        callDuration: { low: 0, medium: 1, high: 2 },
        smsFreq: { low: 0, medium: 1, high: 2 },
        complaintCount: { low: 0, medium: 1, high: 2 },
        topupFreq: { low: 0, medium: 1, high: 2 },
    ...
    const predictionPayload = {
        plan_type: userProfile.planType || 'basic', // Kirim sebagai string, default ke 'basic' jika kosong
    ...
    try {
        ...
        const fullRecommendedProducts = predictionsArray.map(pred => {
            // Backend mengirim id_product sebagai string (misal: "005"), frontend punya id sebagai number (misal: 5).
            // Kita perlu membuat perbandingannya cocok.
            const productFromFrontend = recProducts.find(p => p.id === parseInt(pred.id_product, 10));
            return productFromFrontend; // Mengembalikan data produk lengkap dari frontend
        }).filter(p => p); // Hapus produk yang tidak ditemukan (hasilnya null/undefined)
