...
// Reset profil ke kondisi awal (seperti pertama kali buka halaman)
...
function updateFormWithProfile() {
    ...
    // Set number inputs (tanpa batasan)
    const videoGbInput = document.getElementById('videoGbInput');
    const spendPerGbInput = document.getElementById('spendPerGbInput');
    
    if (videoGbInput) {
        videoGbInput.value = userProfile.videoGbUsage ?? '';
    }
    
    if (spendPerGbInput) {
        spendPerGbInput.value = userProfile.spendPerGb ?? '';
    }
}
...
function calculateRecommendations() {
    const scoredProducts = recProducts.map(product => {
        let score = 0;
        let reasons = [];
        
        // ====== SCORING SESUAI PROFIL ======
        if (userProfile.planType === 'basic') {
            if (
                product.category === 'general' ||
                product.price.includes('25') ||
                product.price.includes('30') ||
                product.price.includes('45') ||
                product.price.includes('55')
            ) {
                score += 25;
            ...
            if (product.category === 'streaming') {
                score += 25;
                reasons.push("Cocok untuk pengguna paket premium (fokus streaming)");
            }
        } else if (userProfile.planType === 'unlimited') {
            if (
                product.name.includes('Unlimited') ||
                product.features.some(f => f.includes('Unlimited'))
            ) {
                score += 25;
        ...
        if (userProfile.topupFreq === 'low') {
            if (product.features.some(f => f.includes('30 Hari') || f.includes('30 Days'))) {
                score += 10;
                reasons.push("Paket bulanan untuk minim top-up");
            }
        ...
        const videoUsage = userProfile.videoGbUsage ?? 0;
        if (videoUsage > 30) {
        ...
        const spendPerGb = userProfile.spendPerGb ?? 0;
...
function setupFormHandlers() {
    ...
    DOM.recForm.addEventListener('submit', (e) => {
        ...
        const formData = {
            ...
            // bebas tanpa batasan: apa pun yg diketik user akan di-parse ke number (kalau kosong → 0)
            videoGbUsage: videoGbInput ? parseFloat(videoGbInput.value || '0') : 0,
            spendPerGb: spendPerGbInput ? parseFloat(spendPerGbInput.value || '0') : 0
        };
    ...
    if (DOM.refreshRecBtn) {
        DOM.refreshRecBtn.addEventListener('click', () => {
            // 1. Hapus profil & rekomendasi dari localStorage
            localStorage.removeItem('insighttel_profile');
            localStorage.removeItem('insighttel_recommendedProducts');

            // 2. Reset objek userProfile ke kondisi awal
            resetUserProfile();

            // 3. Reset form di layar
            if (DOM.recForm) {
                DOM.recForm.reset();
            }
            // sinkronkan lagi radio & input number dengan profil baru
            updateFormWithProfile();

            // 4. Update tampilan profil & rekomendasi
            updateProfileSummary();   // ini akan menyembunyikan card Profil Pengguna (karena planType kosong)
            displayRecommendations(); // ini akan menampilkan card "Belum Ada Profil"

            // 5. Scroll balik ke form input di atas
